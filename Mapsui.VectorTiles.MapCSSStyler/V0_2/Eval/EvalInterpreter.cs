    ...
    public class EvalInterpreter
    {
		...
		public int InterpretInt(string evalFunction, ITagsSource tags)
		{
			...
			if (!int.TryParse (result, out resultInt)) {
                System.Diagnostics.Debug.WriteLine(string.Format("EvalInterpreter: Cannot convert '{0}' to int.", result)); // OsmSharp.Logging.Log.TraceEvent("EvalInterpreter", OsmSharp.Logging.TraceEventType.Error, string.Format ("Cannot convert '{0}' to int.", result));
			}
			return resultInt;
		}
